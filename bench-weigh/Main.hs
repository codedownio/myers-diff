...
testFunc inputSize samples = wgroup [i|#{inputSize} characters|] $ do
  ...
  func' "myers-diff-text-unfoldr" (L.map (\(_, _, initialText, finalText, _, _) -> (VM.diffTextsUnfoldr initialText finalText))) samples
  func' "myers-diff-text-deepseq" (L.map (\(_, _, initialText, finalText, _, _) -> (VM.diffTextsFastNf initialText finalText))) samples
#ifdef DIFF
